	...
	const handlerSubmit = (e) => {
		...
		if (buyer.name !== "" && buyer.phone !== "" && buyer.email !== "") {
			generateOrder(order)
				...
				.then(() => deleteCart())
				.catch((err) => new Swal(`Hubo un Error!`, "Inténtalo nuevamente", "error"))
		} else {
			new Swal({
				...
				<div className="col-md-8">
					...
					{cart.length === 0 && (
						<>
							...
							<Link to="/" className="btn bg-principal text-white d-block w-100 mt-3 fw-bolder fs-5">
								Busquemos algunos <i className="ps-3 fas fa-smile-wink fs-5"></i>
							</Link>
						</>
					)}
				</div>
